shinyjs::disabled(
  shinyWidgets::verticalTabPanel(
    ...
    shiny::tags$table(
      shiny::tags$tr(
        shiny::tags$td(
          shiny::actionButton("optimize_thresholds", "Autothreshold", width = "100%"),
          style = "width: 39%; padding-top: 4px"
        ),
        shiny::tags$td(HTML("&nbsp;with&nbsp;")),
        shiny::tags$td(
          shiny::selectInput("threshold_method_x", "", c(
            "Otsu's method" = "Otsu",
    ...
    shiny::hr(),
    shiny::sliderInput("smooth_x", "Contour smoothing",
      width = "100%", min = 0, max = 10,
      value = 0, step = 0.1
    ),
    shiny::hr()
  )
