...
vignette: >
  %\VignetteIndexEntry{z3_fixing_tracks}
...
This will launch the app either in the viewer pane of RStudio and Positron, or in a separate window, depending on your local configuration. You can control where the app opens by setting the `shiny.launch.browser` option in the `options()` function. For instance:
...
The first step of the fixing process is to load a video file and the tracking data into the app. To do this, simply click the “Select video” button. This will bring up a navigator that you will use to locate the video file that you would like to track. Once you have located the video file in the navigator, click the “Select” button. `trackFixer` will open the video and display its first image in the display window (see below).
...
Once you click the “Save” button in `trackFixer`, the data that is saved in the “\[original_file_name\]\_fixed.csv” file has the same columns has those saved originally by `trackR` (see [here for more details](z2_tracking_objects.html#output)).
